...
func (s *PostgresStore) Create(ctx context.Context, sk *models.SkillManifest) error {
	if exists, err := s.nameVersionExists(ctx, sk.Name, sk.Version, ""); err != nil {
		return err
	} else if exists {
		return ErrConflict
	}
...
func (s *PostgresStore) Update(ctx context.Context, sk *models.SkillManifest) error {
	...
	if exists, err := s.nameVersionExists(ctx, sk.Name, sk.Version, sk.ID); err != nil {
		return err
	} else if exists {
		return ErrConflict
	}
...
func (s *PostgresStore) nameVersionExists(ctx context.Context, name, version, exceptID string) (bool, error) {
	var exists bool
