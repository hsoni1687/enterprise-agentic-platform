...
class WorkflowAgentRun:
    ...
    async def run(self, request: dict) -> str:
        ...
        if not steps_raw:
            # No steps defined — fall back to a single LLM call
            self._emit({"type": "thinking", "content": "No steps defined, running as single LLM call..."})
            result = await workflow.execute_activity(
                "run_single_llm_step",
                args=[prompt, system_prompt, model, agent_id, tenant_id],
                start_to_close_timeout=timedelta(seconds=min(max_duration, 60)),
        ...
        for step in ordered_steps:
            ...
            try:
                result = await _execute_step(
                    ...
                    agent_id, tenant_id, hitl_on_mutating, max_duration,
                )
...
async def _execute_step(
    ...
    if step_type == "llm":
        ...
        return await workflow.execute_activity(
            "run_single_llm_step",
            args=[step_input, system_prompt, model, agent_id, tenant_id],
...
def _resolve_input(step: dict, step_results: dict[str, str]) -> str:
    ...
    if not mapping:
        # Default: use the most recent step result as input
        if step_results:
