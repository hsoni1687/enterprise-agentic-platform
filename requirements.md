...
| **FR26** | **P1** | **Knowledge Graph Visualization & Browse (Agent Studio)** | Agent Studio provides an interactive KG visualization interface within the tenant workspace (port 3000). Domain architects can: (1) Visualize graphs as interactive node-edge diagrams (D3.js/Cytoscape renderer), (2) Search/filter entities by type, properties, or relationships, (3) Click on nodes to inspect properties and connected edges, (4) Traverse relationships (e.g., "show all services that depend on this one"), (5) View KG statistics (entity count by type, relationship count, densest nodes), (6) Export graph as JSON/PNG. Multi-tenant isolation enforced via RLS. |
| **FR27** | **P2** | **Knowledge Graph Management UI (Admin Console)** | Admin Console provides `/knowledge-graphs` page for cross-tenant KG administration: list all tenant KGs, view aggregate KG statistics, inspect specific tenant graphs (read-only), search entities across tenants, view relationship patterns. For operators to understand platform KG usage; architects use Agent Studio visualization instead. |
